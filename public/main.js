...
let server = "https://172.104.100.202:8001"
// let server = ""

async function fetchGetJson(path) {
  let r = await window.fetch(`${server}${path}`)
  return await r.json()
}

async function fetchPostJson(path, obj) {
  let r = await window.fetch(`${server}${path}`, {
...
window.onhashchange = async function () {
  ...
  switch (tokens[0]) {
    case '#show':
      let post = await fetchGetJson('/post/' + tokens[1])
      R.show(post)
      break
    case '#new':
      R.new()
      break
    default:
      let posts = await fetchGetJson('/list')
      R.list(posts)
      break
  }
}
...
R.savePost = async function () {
  ...
  let r = await fetchPostJson('/post', {title: title, body: body})
